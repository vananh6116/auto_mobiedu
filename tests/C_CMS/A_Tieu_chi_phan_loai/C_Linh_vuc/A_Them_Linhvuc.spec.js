...
/**
 * Case 5: Thành công : Thêm mới lĩnh vực - trạng thái Ẩn
 * Mong muốn: Web không hiển thị
 */

function case5 () {
    test('Case 5: Pass - Ẩn ', async ({ page }) => {
    ...
    // Nhập tên lĩnh vực
    await page.getByRole('textbox', { name: 'Tên lĩnh vực *' }).click();
    await page.getByRole('textbox', { name: 'Tên lĩnh vực *' }).fill('QA_LV_Tôn giáo_Case5');
    // Nhập vị trí hiển thị 
    await page.getByRole('spinbutton', { name: 'Vị trí hiển thị *' }).click();
    await page.getByRole('spinbutton', { name: 'Vị trí hiển thị *' }).fill('1');
    // Chọn trạng thái : Hoạt động
    ...
    await page.getByRole('button', { name: 'Lưu' }).click();
    await expect(page.getByText('Thêm thành công!')).toBeVisible();
    await expect(page.getByRole('cell', { name: 'QA_LV_Tôn giáo_Case5' })).toBeVisible();
    // Truy cập Web kiểm tra hiển thị 
    await page.goto(dataSiteTest[1].linkSite + "/cong-thi");
    await expect(page.locator('#desktop-field').getByText('QA_LV_Tôn giáo_Case5')).not.toBeVisible();
    // Truy cập CMS xóa Lĩnh vực mới thêm 
    await page.goto(dataSiteTest[0].linkSite + "/course-field");
    await page
            .locator('tbody > tr')
            .filter({ hasText: 'QA_LV_Tôn giáo_Case5' })
            .locator('i')
            .nth(1)
            .click();
    await page.getByRole('button', { name: 'Xóa' }).click();
