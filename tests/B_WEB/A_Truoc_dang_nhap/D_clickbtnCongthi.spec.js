...
function case1 () {
    test('Case 1: click logo trường mầm non, tiểu học - mobiEdu SLL', async ({ page }) => {
        ...
        await expect(page).toHaveURL(dataSiteTest[1].linkSite + "/cong-thi?category=tre-em")
        ...
        await expect(page).toHaveURL(dataSiteTest[1].linkSite + "/cong-thi?category=hoc-sinh-pho-thong");
        await page.waitForTimeout(2000);
        ...
        await expect(page).toHaveURL(dataSiteTest[1].linkSite + "/cong-thi?category=sinh-vien-nguoi-di-lam");
        await page.waitForTimeout(2000);
...
function case2 () {
    test('Case 2: click checkbox lĩnh vực', async ({ page }) => {
        ...
        // Expect 
        await expect(page).toHaveURL(dataSiteTest[1].linkSite + "/cong-thi?subjects=1,2,3,4,5");
...
function case3 () {
    test('Case 3: click link banner đầu trang', async ({ page }) => {
        ...
        await expect(page).toHaveURL(dataSiteTest[1].linkSite + "/dang-nhap");
        ...
        await page.locator('div.swiper-slide.swiper-slide-active > div > div > div > a > img').click();
        await page.goto('https://mobiedu.vn/tin-tuc/tich-hop-chatgpt-nang-tam-hoc-tap-tren-mobiedu-p2063.html');
        await expect(page.locator('.news-detail-title',{hasText:'TÍCH HỢP CHATGPT - NÂNG TẦM HỌC TẬP TRÊN MOBIEDU'})).toBeVisible();
